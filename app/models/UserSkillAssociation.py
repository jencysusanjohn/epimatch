...
Skill <-> UserSkillAssociation <-> User
"""
...
class UserSkillAssociation(db.Model):
    ...
    skill_id = db.Column(db.Integer, db.ForeignKey(
        'user_skill.id'), primary_key=True)
    skill = relationship('Skill')
    # ---- Other fields ---- #
    endorsements = db.Column(db.Integer, nullable=False)
    ...
    def from_dict(self, data):
        for field in ['endorsements', 'user_id', 'skill_id', 'skill']:
            if field in data:
                setattr(self, field, data[field])

    def __repr__(self):
        return '<UserSkillAssociation {}>'.format(self.id)
